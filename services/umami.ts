...
export const getPageViewsByDataRange = async (domain: string) => {
  ...
  try {
    ...
    if (response.data?.error) {
      console.error(`Umami PV API Error for ${domain}:`, response.data.error);
      return {
    ...
    console.error(`Umami PV Error for ${domain}:`, error?.response?.data || error.message);
    return {
...
export const getWebsiteStats = async (domain: string) => {
  ...
  try {
    ...
    if (response.data?.error) {
      console.error(`Umami Stats API Error for ${domain}:`, response.data.error);
      return {
        status: response.data.error.status || 401,
        data: {
          pageviews: { value: 0 },
    ...
    console.error(`Umami Stats Error for ${domain}:`, error?.response?.data || error.message);
    return {
      status: error?.response?.status || 500,
      data: {
        pageviews: { value: 0 },
