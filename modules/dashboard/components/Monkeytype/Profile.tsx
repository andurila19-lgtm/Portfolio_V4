...
const Profile = ({ data }: ProfileProps) => {
  ...
  const date = data?.addedAt ? new Date(data.addedAt) : new Date();
  const endDate = new Date();
  const durationDays = differenceInDays(endDate, date);

  const timeTyping = data?.typingStats?.timeTyping || 0;
        ...
        <div className="flex gap-x-4">
          <div className="flex items-center">
            <Image
              ...
              className="aspect-square rounded-full border-2 border-neutral-400 object-cover transition-all duration-300 dark:border-neutral-600 lg:hover:scale-105"
            />
          </div>
